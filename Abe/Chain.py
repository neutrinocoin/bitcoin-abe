...
def create(policy, **kwargs):
    ...
    if policy == "Neutrinocoin":		return Neutrinocoin(**kwargs)
...
class Neutrinocoin(LtcScryptChain):
    def __init__(chain, **kwargs):
        chain.name = 'Neutrinocoin'
        chain.code3 = 'NTR'
        chain.address_version = '\x35'
        chain.script_addr_vers = '\x05'
        chain.magic = "\xfd\xc2\xb8\xdd"
        Chain.__init__(chain, **kwargs)

    datadir_conf_file_name = 'neutrinocoin.conf'
    datadir_rpcport = 8776
